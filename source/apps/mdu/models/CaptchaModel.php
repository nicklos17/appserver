...
class CaptchaModel extends ModelBase
{
    ...
    {
        return $this->db->query('SELECT mc_captcha, mc_addtime, mc_validtime '.
        'FROM cloud_mobi_captcha where mc_mobi = ? AND mc_type = ? ORDER BY mc_addtime DESC limit 1',
            array(
                $mobi,
                $type
                )
    ...
    {
        $res = $this->db->query('SELECT `mc_addtime`, `mc_validtime` FROM `cloud_mobi_captcha` '.
        'WHERE `mc_mobi` = ? AND `mc_type` = ? AND `mc_captcha` = ? ORDER BY `mc_addtime` DESC LIMIT 1',
            array(
                $mobi,
                $type,
                $captcha
    ...
    {
        $this->db->execute('UPDATE cloud_mobi_captcha SET mc_validtime= :validtime WHERE mc_mobi= :mobi and mc_captcha=:captcha and mc_type=:type',
            array(
                'mobi'=> $mobi,
                'captcha'=> $captcha,
                'type'=> $type,
